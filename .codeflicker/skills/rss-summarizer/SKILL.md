...
description: 读取 data/pending.yaml 中的原始 RSS 文章列表，对每篇文章生成中文摘要、标签和分类，然后将所有内容汇总并写入 data/temp/digest_batch.yaml，最后调用 Python 脚本生成 Jekyll Chirpy 日报 Markdown 文件（docs/_posts/YYYY-MM-DD-daily-digest.md）。当用户说"生成今天的日报"或"处理待总结文章"时使用此 Skill。
---
...
1. **翻译成中文标题**：如果标题是英文，翻译成中文；如果已经是中文，保持不变。
2. **生成中文摘要**：2-3句话，简明概括核心观点，用中文
3. **打标签**：3-5个中文标签，内容不限于技术，可以是政治、经济、科学、文化、体育、社会等任何领域
4. **判断分类**：根据文章实际内容自由判断，例如：
